...
const StudentList = () => {
  ...
  const [selectedYear, setSelectedYear] = useState("1"); // default semester = 1
  const [groupedStudents, setGroupedStudents] = useState({});

  useEffect(() => {
    if (!students.length) {
      navigate("/"); // Redirect if no students
      return;
    }

    const grouped = students.reduce((acc, student) => {
      const year = student.semester?.toString() || "Unknown";
      if (!acc[year]) acc[year] = [];
      acc[year].push({
        id: student.student_id,
        name: `${student.first_name} ${student.last_name}`,
        roll: `S${student.student_id}`,
        email: student.email,
      });
      return acc;
    }, {});
