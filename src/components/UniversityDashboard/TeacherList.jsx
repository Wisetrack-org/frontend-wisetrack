...
const TeacherList = () => {
  ...
  useEffect(() => {
    if (teachers.length > 0) {
      ...
      const grouped = teachers.reduce((acc, teacher) => {
        const year = teacher.year || "Unknown Year";
        if (!acc[year]) acc[year] = [];
        acc[year].push(teacher);
        return acc;
      }, {});
      setTeachersByYear(grouped);
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-black min-h-screen p-6 md:p-12 text-gray-200">
      ...
      {/* Year buttons */}
      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {Object.keys(teachersByYear).map((year) => (
          <button
            key={year}
      ...
      {/* Teachers grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {teachersByYear[selectedYear]?.map((teacher) => (
          <div
            key={teacher.id}
            className="bg-gray-800 p-5 rounded-xl shadow-lg border border-gray-700 hover:scale-105 transition duration-300"
          >
            <h3 className="text-xl font-bold text-blue-400">{teacher.first_name} {teacher.last_name}</h3>
            {/* <p className="text-gray-300">Qualification: {teacher.qualification}</p>
            <p className="text-gray-400">Subjects: {teacher.subjects}</p> */}
          </div>
        ))}
